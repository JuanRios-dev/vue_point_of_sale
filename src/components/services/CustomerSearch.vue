...
const customerFormData = ref({ tipo_documento: 'CC' });
...
const customerSearch = async () => {
    ...
    searchTimeout = setTimeout(async () => {
        const response = await axios.get(`/customers?search=${customerQuery.value}`);
        customerResults.value = response.data.customers.data;
    }, 300);
}
...
const customerFormCreate = async () => {
    const { status, error, errors } = await sendRequest('POST', '/customers', customerFormData.value);

    if (status === 201) {
        closeModal('modalCreateCustomer');
        customerData.value.numero_documento = customerFormData.value.numeroDocumento;
        tables.clearFormData(customerFormData.value, { tipo_documento: 'CC' });
...
<template>
    ...
    <Modal :modalId="'modalCreateCustomer'" :title="'Formulario de Cliente'">
        <template v-slot:body>
            <CustomerForm :formData="customerFormData" :infoErrors="customersFormErrors"></CustomerForm>
        </template>
        <template v-slot:footer>
