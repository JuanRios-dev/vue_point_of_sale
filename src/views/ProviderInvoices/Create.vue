...
const fetchData = async () => {
    const response = await axios.get(`/cashes?pagination=${0}`);
    cashes.value = response.data.cashes;
}
...
const updateCashId = () => {
    const selectedPaymentName = DataCreate.value.formaPago;

    if (selectedPaymentName !== 'Efectivo' && selectedPaymentName !== 'Nequi') {
        // Encuentra el objeto de caja correspondiente al nombre seleccionado
        const selectedCash = cashes.value.find(cash => cash.nombre === selectedPaymentName);
        if (selectedCash) {
            DataCreate.value.cash_id = selectedCash.id;
        }
    } else {
        // Si la forma de pago seleccionada es "Efectivo" o "Nequi", reinicia DataCreate.cash_id
        DataCreate.value.cash_id = null;
    }
}
...
const DataSave = async () => {
    ...
    if (confirmed) {
        const { status, error, errors } = await sendRequest('POST', '/provider-invoices', DataCreate.value, true, '/provider-invoices', null);
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" :class="{ 'is-invalid': createErrors['codigo'] }"
                        id="codigo" placeholder="codigo" v-model="DataCreate.codigo" autocomplete="off">
                    <span v-if="createErrors['codigo']" class="invalid-feedback">
                        {{ createErrors['codigo'].join(', ') }}
                    </span>
                </div>
            </div>
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <select class="form-control" :class="{ 'is-invalid': createErrors['formaPago'] }" id="formaPago"
                        v-model="DataCreate.formaPago" @change="updateCashId()">
                        <option value="" disabled>Selecciona</option>
                        <optgroup label="Pago por Caja">
                            <option v-for="cash in cashes" :key="cash.id" :value="cash.nombre">{{ cash.nombre }}
                            </option>
                        </optgroup>
